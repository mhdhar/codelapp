...
2. For each theme, count how many distinct reviews, respondents, or deal
   records mention it, plus the source types represented. Do not treat labels
   such as "G2" or "Reddit" as independent sources when several quotes come
   from the same underlying record. Quote the single clearest example.
...
If a theme has only one independent review, respondent, or deal record behind
it, label it "single signal, not a pattern" instead of ranking it. Report the
total sample size and source mix before drawing conclusions.
...
## Real example output
Themes found (12 independent review, survey, or deal records; source mix: G2, Reddit, churn survey):

1. Onboarding time (5 records across G2, Reddit, and churn survey) - why we lose: buyers compare setup time during trial and pick the faster one. Clearest quote: G2 review, 2-star: "Took us three weeks to get the first integration live, competitor had us running in a day." Classification: product-gap hypothesis - setup friction is named directly, but the sample is still limited.

2. Price at scale (4 records across G2 and churn survey) - why customers churn: mentioned mostly by existing customers hitting usage limits, not prospects comparing options. Clearest quote: Churn survey: "Costs tripled once we passed 50 seats, no warning." Classification: perception/messaging hypothesis - validate the pricing-page explanation before treating surprise, rather than price, as the root cause.
